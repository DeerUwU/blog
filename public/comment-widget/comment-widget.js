...
const v_formHtml = `
    ...
    <div id="c_nameWrapper" class="c-inputWrapper">
        <input class="c-input c-nameInput" name="entry.${s_nameId}" id="entry.${s_nameId}" type="text" maxlength="${s_maxLengthName}" placeholder="${s_nameFieldLabel}" required>
    </div>

    <div id="c_websiteWrapper" class="c-inputWrapper">
        <input class="c-input c-websiteInput" name="entry.${s_websiteId}" id="entry.${s_websiteId}" type="url" pattern="https://.*" placeholder="${s_websiteFieldLabel}">
    </div>

    <div id="c_textWrapper" class="c-inputWrapper">
        <textarea class="c-input c-textInput" name="entry.${s_textId}" id="entry.${s_textId}" rows="4" cols="50"  maxlength="${s_maxLength}" placeholder="${s_textFieldLabel}" required></textarea>
    </div>
...
function getComments() {
    ...
    retrievedSheet.then(result => {
        ...
        if (json.table.parsedNumHeaders > 0) { // Check if any comments exist in the sheet at all before continuing
            for (var r = 0; r < json.table.rows.length; r++) {
                // Check for null rows
                let val1;
                ...
                if (val1 == v_pagePath) { 
                    let comment = {}
                    for (var c = 0; c < json.table.cols.length; c++) {
...
function changePage(dir) {
    ...
    v_commentMax = s_commentsPerPage * v_pageNum;
    v_commentMin = v_commentMax - s_commentsPerPage;
    for (var i = 0; i < a_commentDivs.length; i++) {
